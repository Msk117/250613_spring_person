...
function spreadCommentList(bno, page = 1) {
    getCommentListFromServer(bno, page).then(result =>{
        ...
        if (result.cmtList.length > 0 ) {
            ...
            for (let cvo of result.cmtList) {
                let li = `<li class="list-group-item" data-cno="${cvo.cno}">`;
                    ...
                    if (cvo.writer === loginNick) {
                        li += `<button type="button" class="btn btn-outline-warning btn-sm mod">%</button>`;// 수정
                        li += `<button type="button" class="btn btn-outline-danger btn-sm del">X</button>`; // 삭제  
            ...
            // 더보기 버튼이 표시되는 조건
            // ph =>pgvo.pageNo < realEndPage => 더보기 버튼 표시
...
document.addEventListener('click',(e)=>{
    ...
    if(e.target.id==='moreBtn'){
        let page = parseInt(e.target.dataset.page); // 문자로 인지. => 숫자로 인지
        spreadCommentList(bnoValue, page);
    }

    // 수정일 경우 cno, content => 서버로 전송하여 update 
